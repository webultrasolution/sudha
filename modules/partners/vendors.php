...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add') {
        $stmt = $pdo->prepare("INSERT INTO partners (name, contact_person, phone, email, address, city, state, gstin, pan, billing_address, payment_terms, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'vendor')");
        $stmt->execute([$name, $contact, $phone, $email, $address, $city, $state, $gstin, $pan, $billing_address, $payment_terms]);
        header("Location: vendors.php?msg=added"); exit;
    } elseif ($_POST['action'] === 'edit') {
        $id = intval($_POST['id']);
        $stmt = $pdo->prepare("UPDATE partners SET name=?, contact_person=?, phone=?, email=?, address=?, city=?, state=?, gstin=?, pan=?, billing_address=?, payment_terms=? WHERE id=?");
        $stmt->execute([$name, $contact, $phone, $email, $address, $city, $state, $gstin, $pan, $billing_address, $payment_terms, $id]);
