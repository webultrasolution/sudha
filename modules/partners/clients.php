...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add') {
        ...
        $trashId = move_row_to_trash($pdo, 'partners', 'id', $id, $_SESSION['user_id'] ?? null, 'Client deleted via UI');
        if ($trashId) {
            echo json_encode(['success' => true]);
        } else {
            echo json_encode(['success' => false, 'message' => 'Failed to move client to trash.']);
        }
        exit;
    }
}
