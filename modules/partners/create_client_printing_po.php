...
$sites = $pdo->query("SELECT id, name, site_code, width, height, city, state, type, light_type, owner_type, status FROM sites ORDER BY site_code ASC")->fetchAll();
                    ...
                    <label id="site_label" style="display: flex; justify-content: space-between; align-items: center; font-weight: 800; color: #1e293b; font-size: 0.75rem; margin-bottom: 1rem; text-transform: uppercase; letter-spacing: 0.05em;">
                        ...
                        <div id="multi_site_container" style="flex: 1; display: flex; flex-direction: column;">
                            ...
                            <div id="site_checkbox_list" class="site-grid" style="flex: 1; max-height: 480px; overflow-y: auto; border: 1px solid #f1f5f9; border-radius: 12px; background: #fff;">
                                <div style="grid-column: 1 / -1; color: #94a3b8; font-size: 0.9rem; padding: 4rem; text-align: center; font-style: italic;">Select a client to see sites...</div>
                            </div>
                        </div>
                    </div>
                </div>
...
function filterSitesByClient() {
    ...
    displaySites.forEach(s => {
        ...
        item.innerHTML = `
            ...
            <div class="site-info">
                <label>${s.site_code} - ${s.name}</label>
                <small>${s.width}x${s.height} = <strong>${parseFloat(s.width) * parseFloat(s.height)} SQFT</strong></small>
            </div>
...
function filterSitesInModal() {
    ...
    document.querySelectorAll('.site-item').forEach(item => {
        ...
        if (matchesSearch && matchesAvailability && matchesMedia && matchesState && matchesCity && matchesLight && matchesSize) {
...
document.getElementById('rateForm').addEventListener('submit', function(e) {
    ...
    if (checked.length === 0) {
        ...
        Swal.fire({
            title: 'Error',
            text: 'Please select at least one site to save this Client PO.',
            icon: 'error',
